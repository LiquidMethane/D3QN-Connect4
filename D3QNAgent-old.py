...
class Agent():
    def __init__(self, lr, gamma, n_actions, epsilon, batch_size,
                 ...
                 fc2_dims=128, replace=100, train=True):
    ...
    def choose_action(self, observation, config):
        # acquire columns that are still empty
        mask = [True if c == 0  else False for c in observation[0:7]]
        ...
        if self.train and np.random.random() < self.epsilon:
            ...
            valid_actions_mask = tf.boolean_mask(actions, mask)
            action = tf.math.argmax(valid_actions_mask, axis=1).numpy()[0]
        return action

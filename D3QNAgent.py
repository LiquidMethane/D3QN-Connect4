...
class Agent():
    ...
    def __init__(self, config, lr, gamma, batch_size, epsilon, 
        ...
        self.model = D3QN(self.n_actions, d1_dims, d2_dims)
        self.target = D3QN(self.n_actions, d1_dims, d2_dims)
    ...
    def choose_action(self, observation):
        ...
        mask = [True if observation[idx] == 0 else False \
        ...
        if np.random.random() < self.epsilon:
            ...
            valid_advantages = [advantages[idx] 
                                if mask[idx] else np.NaN
                               for idx in range(self.n_actions)]
